...
import { Plus, Eye, Edit, Trash2, Download, Upload, Home as HomeIcon } from 'lucide-react';
...
import { getRentCycles, updateRentCycle, deleteRentCycle } from '../../lib/rentCycle';
 ...
 */
export const RentCyclesWorkspaceV4: React.FC<RentCyclesWorkspaceV4Props> = ({
  user,
  onNavigate,
  onStartNew,
  onEditCycle,
  ...
  const allCycles = useMemo(() => {
    return getRentCycles(user.role === 'admin' ? undefined : user.id, user.role);
  }, [user.id, user.role]);
  ...
  // Calculate stats
  const stats = useMemo(() => {
    const available = allCycles.filter(c => c.status === 'available').length;
    const active = allCycles.filter(c => c.status === 'active').length;
    const applications = allCycles.reduce((sum, c) => 
      sum + (c.applications?.filter(app => app.status === 'pending').length || 0), 0
    );
  ...
  const columns: Column<RentCycle>[] = [
    ...
    },
    {
      ...
      accessor: (c) => (
        <div>
          {c.currentTenantName ? (
            <>
              <div className="text-sm text-gray-900">{c.currentTenantName}</div>
              <div className="text-xs text-gray-500">Tenant</div>
            </>
          ) : (
            <>
              <div className="text-sm text-gray-900">{c.landlordName}</div>
              <div className="text-xs text-gray-500 capitalize">{c.landlordType}</div>
            </>
          )}
        </div>
      ),
    ...
    },
    {
      ...
      accessor: (c) => (
        <div className="text-sm text-gray-900">
          {c.leaseEndDate ? (
            <>
              {new Date(c.leaseEndDate).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric',
              })}
            ...
            <>
              Available {new Date(c.availableFrom).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
              })}
            </>
    ...
    },
    {
      ...
      label: 'Applications',
      accessor: (c) => {
        const pendingApps = c.applications?.filter(app => app.status === 'pending').length || 0;
        return (
          <div className="text-sm text-gray-900">
            {pendingApps > 0 ? (
              <span className="text-yellow-600">{pendingApps} pending</span>
            ) : (
  ...
  const quickFilters = [
    {
      ...
      options: [
        ...
        { value: 'application-received', label: 'Application Received', count: allCycles.filter(c => c.status === 'application-received').length },
        ...
        { value: 'renewal-pending', label: 'Renewal Pending', count: allCycles.filter(c => c.status === 'renewal-pending').length },
    ...
    },
    {
      ...
      label: 'Landlord Type',
      options: [
        { value: 'agency', label: 'Agency', count: allCycles.filter(c => c.landlordType === 'agency').length },
        { value: 'client', label: 'Client', count: allCycles.filter(c => c.landlordType === 'client').length },
        { value: 'investor', label: 'Investor', count: allCycles.filter(c => c.landlordType === 'investor').length },
      ],
      multiple: true,
    },
    {
    ...
    },
    {
      ...
      options: [
        { 
          value: 'occupied', 
          label: 'Occupied', 
          count: allCycles.filter(c => c.currentTenantName).length 
        },
        { 
          value: 'vacant', 
          label: 'Vacant', 
  ...
  const handleFilter = (cycle: RentCycle, filterValues: Record<string, any>): boolean => {
    // Status filter
    ...
    if (filterValues.landlordType?.length > 0 && !filterValues.landlordType.includes(cycle.landlordType)) {
      return false;
    }

    // Lease period filter
    if (filterValues.leasePeriod?.length > 0) {
      const periods = filterValues.leasePeriod.map((p: string) => parseInt(p));
    ...
    if (filterValues.tenantStatus) {
      ...
      if (filterValues.tenantStatus === 'occupied' && !isOccupied) return false;
      if (filterValues.tenantStatus === 'vacant' && isOccupied) return false;
    }
  ...
  const handleSort = (cycles: RentCycle[], sortBy: string): RentCycle[] => {
    const sorted = [...cycles];
    ...
    switch (sortBy) {
      ...
      case 'lease-ending':
        sorted.sort((a, b) => {
          const endA = a.leaseEndDate ? new Date(a.leaseEndDate).getTime() : Infinity;
          const endB = b.leaseEndDate ? new Date(b.leaseEndDate).getTime() : Infinity;
          return endA - endB;
        });
        break;
      default:
  ...
  const handleSearch = (cycle: RentCycle, query: string): boolean => {
    const property = getProperty(cycle.propertyId);
    const searchLower = query.toLowerCase();
    
    return (
      cycle.landlordName.toLowerCase().includes(searchLower) ||
      (cycle.currentTenantName && cycle.currentTenantName.toLowerCase().includes(searchLower)) ||
  ...
  return (
    <WorkspacePageTemplate
      ...
      primaryAction={{
        ...
        onClick: onStartNew || (() => toast.info('Start Rent Cycle clicked')),
      }}
