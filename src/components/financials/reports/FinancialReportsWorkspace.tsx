...
export const FinancialReportsWorkspace: React.FC<FinancialReportsWorkspaceProps> = ({
  ...
  const enhancedTemplates = useMemo(() => {
    return REPORT_TEMPLATES.map(template => {
      const templateReports = generatedReports.filter(r => r.templateId === template.id);
      const lastGenerated = templateReports.length > 0
        ? templateReports.sort((a, b) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime())[0].generatedAt
        : undefined;
  ...
  const metrics = useMemo(() => {
    ...
    const thisMonthReports = generatedReports.filter(r => {
      const reportDate = new Date(r.generatedAt);
      return reportDate.getMonth() === thisMonth && reportDate.getFullYear() === thisYear;
    }).length;

    const sortedReports = [...generatedReports].sort((a, b) => 
  ...
  const handleToggleFavorite = (templateId: string) => {
    if (favoriteReportIds.includes(templateId)) {
      saveFavorites(favoriteReportIds.filter(id => id !== templateId));
        ...
        quickFilters={[
          {
            ...
            type: 'multi-select',
            options: [
              { value: 'financial', label: 'Financial', count: enhancedTemplates.filter(t => t.category === 'financial').length },
              { value: 'operational', label: 'Operational', count: enhancedTemplates.filter(t => t.category === 'operational').length },
              { value: 'custom', label: 'Custom', count: enhancedTemplates.filter(t => t.category === 'custom').length },
            ],
