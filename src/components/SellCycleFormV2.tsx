...
const step1ValidationRules = {
  sellerId: compose(required.withMessage('Please select a seller')),
  sellerName: compose(required.withMessage('Seller name is required')),
};

const step2ValidationRules = {
  askingPrice: compose(
    required.withMessage('Asking price is required'),
    positiveNumber.withMessage('Price must be a positive number'),
    minValue.rule(1, 'Price must be greater than 0')
  ),
};

const step3ValidationRules = {
  commissionRate: compose(
    required.withMessage('Commission rate is required'),
    positiveNumber.withMessage('Commission rate must be a positive number')
  ),
};
...
function Step4AdditionalDetails({
  formData,
  errors,
  onFieldChange,
}: Step4Props) {
