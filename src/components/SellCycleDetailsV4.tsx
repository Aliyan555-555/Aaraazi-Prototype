...
export function SellCycleDetailsV4({
  ...
  const highestOffer =
    cycle.offers?.reduce((max, offer) => Math.max(max, offer.offerAmount), 0) || 0;
  ...
  const allOffersSorted = useMemo(() => {
    return [...(cycle.offers || [])].sort(
      (a, b) => new Date(b.offeredDate).getTime() - new Date(a.offeredDate).getTime()
    );
  }, [cycle.offers]);
  ...
  const handleAcceptOffer = (offerId: string) => {
    if (
      ...
      try {
        ...
        toast.error(error instanceof Error ? error.message : 'Failed to accept offer and create deal');
      }
    }
  };
  ...
  const offersColumns: DataTableColumn<Offer>[] = [
    {
      ...
      render: (offer) =>
        new Date(offer.offeredDate).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'short',
          day: 'numeric',
        }),
  ...
  const activities: Activity[] = useMemo(() => {
    ...
    cycle.offers?.forEach((offer) => {
      activityList.push({
        ...
        description: `${formatPKR(offer.offerAmount)} from ${offer.buyerName}`,
      ...
      if (offer.status === 'accepted') {
        activityList.push({
          ...
          description: `Accepted offer of ${formatPKR(offer.offerAmount)}`,
          date: offer.offeredDate,
        ...
        activityList.push({
          ...
          description: `Rejected offer of ${formatPKR(offer.offerAmount)}`,
          date: offer.offeredDate,
