...
function detectStaledLeads(
  ...
  const staledLeads = leads.filter(lead => {
    ...
    // Find last interaction for this lead
    const leadInteractions = interactions
      .filter(i => i.contactId === lead.contactId)
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

    if (leadInteractions.length === 0) {
      // No interactions - definitely stale
      return new Date(lead.createdAt) < threeDaysAgo;
    }

    const lastInteraction = new Date(leadInteractions[0].date);
    return lastInteraction < threeDaysAgo;
  });
...
function detectBestPerformingDay(
  ...
  [...properties, ...leads, ...interactions as any[]].forEach(item => {
...
export function detectInsights(data: {
  ...
  const insights: (Insight | null)[] = [
    detectStaledLeads(leads, interactions),
    detectSlowResponseTime(leads, interactions),
    ...
    detectBestPerformingDay(properties, leads, interactions),
