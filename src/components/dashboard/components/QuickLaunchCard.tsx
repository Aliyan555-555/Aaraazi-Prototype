    ...
    >
      ...
      {badge && (
        <div className="absolute top-3 right-3">
          <span className="bg-[#C17052] text-white px-2 py-0.5 rounded text-xs">
            {badge}
          </span>
        </div>
      )}
      ...
      {/* Description */}
      {!isCompact && (
        <p className="text-[#363F47] text-sm mb-3 line-clamp-2">
          {description}
        </p>
      )}
      ...
      <div className={`flex items-center justify-between ${isCompact ? 'mt-2' : 'mt-4'}`}>
        ...
        <div className="flex items-center gap-2">
          {recentCount !== undefined && recentCount > 0 ? (
            <span className="text-[#2D6A54] text-sm">
              {recentCount} {recentLabel}
            </span>
          ) : keyboardShortcut ? (
            <span className="text-[#6B7280] text-xs px-2 py-1 bg-gray-100 rounded font-mono">
              {keyboardShortcut}
            </span>
          ) : (
            <span className="text-[#6B7280] text-sm">Quick access</span>
          )}
        </div>
